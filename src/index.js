export default {
  async fetch(request, env, ctx) {
    ...
    function getSubscriptionTraffic(sub) {
      if (!sub) return 0;
      // Если есть сохранённый трафик — используем его
      if (sub.traffic !== undefined) {
        return sub.traffic;
      }
      // Иначе считаем с момента создания
      ...
      if (hasSession) {
        if (method === 'POST') {
          ...
          if (action === 'reset_traffic') {
            // Сброс трафика у всех подписок
            for (const key in subscriptions) {
              subscriptions[key].traffic = 0;
              // Обновляем createdAt, чтобы трафик начал считаться заново
