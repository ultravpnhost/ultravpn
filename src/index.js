...
async function handleRequest(request) {
  ...
  // Проверяем, запрашивает ли клиент подписку напрямую или это VPN-приложение
  ...
  // 1. ВАРИАНТ ДЛЯ VPN КЛИЕНТА (Отдаем массив полных рабочих Xray-конфигов в JSON)
  if (isVpnClient || url.searchParams.get("sub") === "1" || request.headers.get("accept")?.includes("json")) {
    ...
    // "Ultra VPN 🏳️" закодировано в Base64 без багов
    newHeaders.set("profile-title", "base64:VWx0cmEgVlBOIOKavw==");
    
    // Информация о подписке (357 ГБ трафика, лимиты, дата окончания)
  ...
  // 2. ВАРИАНТ ДЛЯ БРАУЗЕРА (Красивая HTML страница подписки)
  const htmlContent = `
  <!DOCTYPE html>
  <html lang="ru">
  <head>
